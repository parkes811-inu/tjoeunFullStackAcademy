...
class FirstScreen extends StatelessWidget {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
          onPressed: () {
            ...
            //             const SecondScreen(data: '넘겨줄 데이터')));
  ...
  Route _createRoute() {
    return PageRouteBuilder(
      pageBuilder: (context, animation, sceondaryAnimation) =>
          const SecondScreen(data: '데이터'),

      // 애니메이션 지정
      transitionsBuilder: (context, animation, secondaryAnimation, child) {
        // Offest(x축, y축) : 0.0 ~ 1.0
        const begin = Offset(1.0, 0.0); // 시작 지점
        const end = Offset.zero; // 종료 지점
        const curve = Curves.bounceIn; // 애니메이션 속도 곡선
